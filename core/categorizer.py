...
class TransactionCategorizer:
    ...
    def _compile_regex_cache(self):
        ...
        for category, keywords in self.category_patterns.items():
            self.regex_cache[category] = [re.compile(r'\b' + re.escape(kw) + r'\b', re.IGNORECASE) for kw in keywords]
    ...
    def multi_pass_categorization(self, description: str, amount: float, date: datetime = None) -> str:
        ...
        if amount > 0:
            income_keywords = [
                'deposit', 'payroll', 'salary', 'refund', 'reversal',
        ...
        credit_card_keywords = [
            'payment thank you', 'online payment', 'autopay',
    ...
    def add_custom_rule(self, keyword: str, category: str):
        """Add a custom rule for categorization"""
        self.custom_rules[keyword.lower()] = category

    def learn_from_feedback(self, description: str, category: str):
        """Placeholder for learning from user feedback"""
        self.learning_data[description.lower()] = category
